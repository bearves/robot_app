#ifndef UPSTEPCLIMB_PLANNER_H
#define UPSTEPCLIMB_PLANNER_H
...
{
    // for recovery
    struct UpStepClimbParam final : aris::server::GaitParamBase
    {
    public:
        double step_length;
        double step_height;
    ...
    class UpStepClimbPlanner
    {
    public:
        static void setMotionSelector(const aris::server::MotionSelector &selector);
        static int  upstepclimb(aris::model::Model &model, aris::server::PlanParamBase &param);
        static bool upstepclimbParser(const std::string &cmd,
    ...
    private:
        ...
        static double foot_hold_key_points_[kinematics::TOTAL_CLIMB_UP_NUMBER+1][kinematics::LEG_NUM*3];
        static double body_pos_key_points_[kinematics::TOTAL_CLIMB_UP_NUMBER+1][6];
        static double distance_of_step_[kinematics::TOTAL_CLIMB_UP_NUMBER][kinematics::LEG_NUM];
        static double height_of_step_[kinematics::TOTAL_CLIMB_UP_NUMBER][kinematics::LEG_NUM];
        static double h2;
        static double distance_of_body_move_[kinematics::TOTAL_CLIMB_UP_NUMBER];
        static double height_of_body_move_[kinematics::TOTAL_CLIMB_UP_NUMBER];
        static double body_pitch_[kinematics::TOTAL_CLIMB_UP_NUMBER];
        static double Delta_d_[kinematics::TOTAL_CLIMB_UP_NUMBER];
